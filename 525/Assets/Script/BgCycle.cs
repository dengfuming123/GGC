...
public class BgCycle : MonoBehaviour
{
    ...
    {
        float before = target.position.y;//上次位置
        float now;//本次位置
        float distance;//背景每帧前进的距离
        while (flag)
        {
            yield return null;
            now = target.position.y;
    ...
    IEnumerator cycleBG()//背景循环协程
    {
        while (flag)
        {
            while (target.transform.position.x < backgrounds[firstLocation].transform.position.x)
                leftCycle();
            while (target.transform.position.x > backgrounds[lastLocation].transform.position.x)
                rightCycle();
